...
public protocol BackupStorageService {
...
public actor DefaultBackupConfigurationService: BackupConfigurationService {
...
public actor DefaultBackupExecutionService: BackupExecutionService {
    ...
    public func startBackup(configurationId: String) async throws -> String {
        ...
        let task = Task {
            do {
                try await executeBackup(job: job, config: config)
            } catch {
    ...
    private func executeBackup(job: BackupJob, config: BackupConfiguration) async throws {
        ...
        do {
            ...
            updatedJob.progress.objectsProcessed = backedUpObjects.count
            ...
            try await auditLogger.store(entry: AuditLogEntry(
                ...
                metadata: [
                    ...
                    "bytes_backed_up": String(backedUpObjects.reduce(0) { $0 + $1.size }),
